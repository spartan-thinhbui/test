...
"""
This module is an updated pipeline where SAM works only within the bounding box provided by efficientnet detector
"""
...
def combine_masks(masks, output_mask=None, bounding_mask=None, resize_mask=None):
    """
    Combine all predicted masks into one
    Args:
        masks (np.array): binary masks with shape [N, 1, H, W]
        output_mask (str): Path to output mask file
        bounding_mask (np.array): Bounding mask that predicted masks must intersect
        resize_mask (tuple[int, int]): Output mask size
    Returns:
        np.array: The combined mask or None if no valid masks
    """
    # Return None if masks is None or empty
    ...
    if bounding_mask is None:
        ...
        # Get shape from first mask or bounding mask
        shape = masks[0][0].shape if masks.shape[0] > 0 else bounding_mask.shape
        # create empty mask
        combined_mask = np.zeros(shape, dtype=np.uint8)
            
        for i in range(masks.shape[0]):
            mask = masks[i]
            if mask.size == 0:  # Skip empty masks
                continue
            contours, _ = cv2.findContours(np.uint8(mask[0]), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            for cnt in contours:
                object_mask = np.zeros_like(mask[0], dtype=np.uint8)  # Empty mask
                cv2.drawContours(object_mask, [cnt], -1, color=1, thickness=cv2.FILLED)  # Draw filled contour

                # check if mask intersects with the bounding_mask
                if check_mask_intersection(object_mask, bounding_mask, ratio=0.01):
                    combined_mask += object_mask

    # Return None if no valid masks were combined
    ...
    if resize_mask:
        combined_mask = cv2.resize(combined_mask, resize_mask, interpolation=cv2.INTER_LINEAR)

    if output_mask:
        cv2.imwrite(output_mask, (combined_mask * 255))
...
def preprocess_mask(mask: np.ndarray, target_size: tuple = None) -> np.ndarray:
    ...
    """
    # Handle None mask by returning zero mask
    if mask is None:
...
def find_mask_bounding_boxes(mask: np.array, padding: int=0, min_area: int=0) -> list:
    """
    Find bounding boxes of a mask
    Args:
        mask: numpy array of any shape with boolean or numeric values
        padding: padding adding the output bounding boxes
        min_area: min area for a bounding box
    Returns:
    ...
    # filter bounding boxes and covert to xyxy format
    bounding_boxes = [
        [
            max(box[0] - padding, 0),
            max(box[1] - padding, 0),
            min(box[0] + box[2] + padding, w),
            min(box[1] + box[3] + padding, h),
        ]
...
class UNetModel(Model):
    ...
    def predict(self, image: np.ndarray, output_dir=None):
        ...
        # Preprocess image
        processed_image = albu_normalize(image=np.array(image))['image']
...
class SAMModel(Model):
    ...
    def predict_by_bboxes(self, image: np.ndarray, bboxes: np.array) -> np.array:
        """
        Args:
        ...
        try:
            ...
            return [[] for _ in range(len(bboxes))]
...
class SegmentationPipeline:
    ...
    def predict(self, image: np.ndarray, return_intermediate: bool = False):
        ...
        sam_image = preprocess_image(image, self.MAX_INPUT_SIZE)
        sam_bound_mask = preprocess_mask(unet_mask, self.MAX_INPUT_SIZE)
        building_bboxes = find_mask_bounding_boxes(sam_bound_mask, padding=5, min_area=25) # add these params to config
        
        # Run SAM prediction
        sam_masks = self.sam.predict_by_bboxes(
            sam_image,
            bboxes=building_bboxes
        )
...
if __name__ == "__main__":
    ...
    for filename in Path(data_dir).glob("*.jpg"):
        ...
        json.dump(contours, open(os.path.join(polygon_dir, base_filename.replace(".jpg", ".json")), "w"))
        ...
        annotate_and_save_image(
            ...
            final_mask,
            contours,
            os.path.join(annotations_dir, base_filename)
        )
        
        # Save the final mask as an image (scaling binary mask to 0-255 for saving)
        save_mask(final_mask, os.path.join(masks_dir, base_filename))
